...
// New Printer pass
struct MultiplicationShiftsPrinter
    : public PassInfoMixin<MultiplicationShiftsPrinter> {
  PreservedAnalyses run(Function &F, FunctionAnalysisManager &) {
...
PassPluginLibraryInfo getMultiplicationShiftsPluginInfo() {
  return {
      LLVM_PLUGIN_API_VERSION, "MultiplicationShifts", LLVM_VERSION_STRING,
      [](PassBuilder &PB) {
        PB.registerPipelineParsingCallback(
            [](StringRef Name, FunctionPassManager &FPM,
              ...
              if (Name == "multiplication-shifts") {
                FPM.addPass(
                    MultiplicationShifts()); // Run the transformation pass
                FPM.addPass(
                    MultiplicationShiftsPrinter()); // Run the printer pass with
                                                    // the modified status
                return true;
              }
              return false;
        ...
        PB.registerPipelineStartEPCallback([](ModulePassManager &MPM,
          ...
          FPM.addPass(
              MultiplicationShiftsPrinter()); // Run the printer pass with the
                                              // modified status
